...
export class TokenService implements ITokenService {
    ...
    private getSecretKey(type: TokenType): string {
        if (type === TokenType.ACCESS) {
            const accessSecretKey = this.configService.getOrThrow<string | undefined>('JWT_ACCESS_SECRET_KEY')
            return accessSecretKey
        } else if (type === TokenType.REFRESH) {
            const refreshSecretKey = this.configService.getOrThrow<string | undefined>('JWT_REFRESH_SECRET_KEY')
    ...
    private getExpiresIn(type: TokenType): string {
        if (type === TokenType.ACCESS) {
            const accessExpiresIn = this.configService.getOrThrow<string | undefined>('JWT_ACCESS_EXPIRES_IN')
            return accessExpiresIn
        } else if (type === TokenType.REFRESH) {
            const refreshExpiresIn = this.configService.getOrThrow<string | undefined>('JWT_REFRESH_EXPIRES_IN')
